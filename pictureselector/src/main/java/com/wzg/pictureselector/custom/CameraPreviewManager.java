...
//        cameraRotation = SingleBaseConfig.getBaseConfig().getVideoDirection();
//        int detectDirection = SingleBaseConfig.getBaseConfig().getDetectDirection();
        Log.e(TAG, "startPreview: 摄像头预览角度 " + cameraRotation /* +" detectDirection: "+detectDirection*/);
        openCamera();
    }
    ...
    private void openCamera() {
        try {
            Log.e(TAG, "openCamera: 啥情况啊1");
            if (mCamera == null) {
                Log.e(TAG, "openCamera: 啥情况啊2");
                Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
                Log.e(TAG, "openCamera: 啥情况啊3" + Camera.getNumberOfCameras());
                for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
                    Camera.getCameraInfo(i, cameraInfo);
                    Log.e(TAG, "openCamera: 能获取到得" + cameraInfo.facing);
...
//                    cameraRotation = getCameraDisplayOrientation(cameraRotation, cameraId);
            ...
            for (Camera.Size size : sizeList) {
                Log.e(TAG, "openCamera: 支持的分辨率daxiao" + size.width + "--" + size.height);
            }
            ...
            Log.e(TAG, "openCamera: videoWidth" + videoWidth + "videoHeight" + videoHeight);
